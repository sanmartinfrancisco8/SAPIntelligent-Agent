...
type ResultPayload = {
  type: 'summary' | 'mind-map' | 'process-flow';
  data: string;
  title: string;
};
  ...
  const handleDownload = () => {
    if (payload?.data) {
        if (payload.type === 'summary' || payload.type === 'mind-map') {
            ...
            a.href = url;
            a.download = `${payload.title.replace(/ /g, '_')}.txt`;
  ...
  if (!payload) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-background text-center text-foreground p-4">
        ...
        <p className="text-muted-foreground mb-6">Cierra esta pestaña y genera un nuevo resultado desde el Área de Estudio.</p>
