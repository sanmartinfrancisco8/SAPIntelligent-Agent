...
import type { ViewerPayload } from '@/components/module/KnowledgeProcessingToolkit';
...
function ViewerComponent() {
  ...
  useEffect(() => {
    ...
    if (key) {
      try {
        const storedPayload = localStorage.getItem(key);
        if (storedPayload) {
          setPayload(JSON.parse(storedPayload));
          // Clean up the item from localStorage after retrieving it
          localStorage.removeItem(key);
        }
      } catch (error) {
        console.error("Failed to parse result payload from localStorage", error);
      } finally {
        setIsLoading(false);
      }
    } else {
  ...
  const handleDownload = () => {
    ...
    const fileSafeTitle = payload.title.replace(/[\\/:\s]/g, '_');
  ...
  if (!payload) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-background text-center text-foreground p-4">
        ...
        <p className="text-muted-foreground mb-6">La sesión ha expirado o el enlace es incorrecto. Cierra esta pestaña y genera un nuevo resultado.</p>
...
export default function ViewerPage() {
    return (
        <Suspense fallback={<div className="flex h-screen w-full items-center justify-center bg-background"><LoadingSpinner size={48} /></div>}>
            <ViewerComponent />
        </Suspense>
    )
}
